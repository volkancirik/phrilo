...
class BAN(nn.Module):
  ...
  def forward(self, sentence, pos_tags, box_reps, gt_chunks, gt_alignments, debug=False):
    ...
    for i in range(n_chunks):
      ...
      gold = makevar(np.array([expected]),
                     numpy_var=True)
      ...
      pred_alignments[phrases[i]].append(argmax_prediction)

      loss += self.criterion(al_score, gold)

      score_np_batch[0, i*n_boxes: (i+1) *
                     n_boxes] = al_score.data.cpu().numpy()
      scores_batch.append(al_score)

    # score_batch = torch.cat(scores_batch, 1)
    # decoder_alignments = self.decoder.solve(
    #     score_np_batch, n_chunks, n_boxes)

    # pred_score = 0.0
    # for box in decoder_alignments[0]:
    #   for chunk in decoder_alignments[0][box]:
    ...
    #     chunks += 1
    # loss = loss + (pred_score - gt_score)*0.0001
    return loss, [], pred_alignments
